...
import { 
  Plus, Search, Trash2, MoreHorizontal, Film, MoveUp, MoveDown
} from 'lucide-react'
import {
  DropdownMenu,
  DropdownMenuContent,
...
export default function NewReleasePage() {
  ...
  const fetchSectionItems = async () => {
    try {
      const response = await fetch('/api/sections/new-release')
      if (response.ok) {
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('确定要从New Release中移除这部影片吗？')) return

    try {
      const response = await fetch(`/api/sections/new-release/${id}`, {
        method: 'DELETE'
      })

      if (response.ok) {
        toast.success('已从New Release中移除')
        fetchSectionItems()
        fetchAvailableMovies()
      } else {
        toast.error('移除失败')
  ...
  const moveOrder = async (id: string, direction: 'up' | 'down') => {
    try {
      const response = await fetch(`/api/sections/new-release/${id}/move`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">New Release 管理</h1>
          <p className="text-gray-600">手动配置New Release板块显示的影片</p>
        </div>
      ...
      {items.length === 0 ? (
        <Card>
          <CardContent className="pt-6">
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-4">选择影片添加到New Release板块</p>
              <Button onClick={() => setIsSelecting(true)} disabled={filteredAvailableMovies.length === 0}>
                <Plus className="h-4 w-4 mr-2" />
                添加影片
              </Button>
        ...
        <div className="space-y-4">
          <div className="text-sm text-gray-600 mb-4">
            当前共 {items.length} 部影片，按顺序显示在前台New Release板块
          </div>
